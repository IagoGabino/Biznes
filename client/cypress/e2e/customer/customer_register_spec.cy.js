...
describe('Customer Registration', () => {
    ...
    it('Should not register a new customer with an already registered email', () => {
      // intercept declaration
      cy.intercept('POST', 'http://localhost:3030/api/customer').as('registerCustomer');
  
      const randomEmail = `${Math.random().toString(36).substring(2, 15)}@example.com`;
      cy.registerCustomer(randomEmail);
    ...
    it('Should not register a customer who typed the wrong password confirmation', () => {
      // intercept declaration
      cy.intercept('GET', 'https://viacep.com.br/ws/30350230/json/').as('getCep');
      cy.intercept('POST', 'http://localhost:3030/api/customer').as('registerCustomer');
      ...
      // wait for the response from the API and check the status code
      cy.get('@registerCustomer').should('not.exist');
    });
});